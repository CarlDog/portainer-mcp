...
| `portainer_convert_stack_to_file` | GET stack (noRedact) → GET file (noRedact) → PUT `/api/stacks/{id}` | Intentionally uses the file-update handler's `GitConfig = nil` behavior to detach in place; preserves stack ID and env server-side; two-factor confirm; no delete/recreate window |
