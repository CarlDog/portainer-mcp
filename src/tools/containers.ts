...
export function registerContainerTools(
  ...
  server.registerTool(
    "portainer_recreate_container",
    {
      ...
      description:
        "Pull the image and recreate a single container, preserving its Config and HostConfig (env, mounts, networks, restart policy, etc). Cleaner than stack-redeploy for 'update one service after pushing a new image' workflows. The old container is stopped and removed; the new one keeps the same name and resource controls. Does not prune images automatically: a dangling digest may be an intentional rollback point, so inspect with portainer_list_images and call confirm-gated portainer_prune_images separately when cleanup is wanted. TIMEOUT RISK: image pull + stop + remove + create all happen inline before this call returns, so a large image over a slow connection may exceed the MCP client's request window while continuing server-side. Verify with portainer_get_container rather than blindly retrying. For a large image, call portainer_pull_image first, then recreate with pull_image: false so a timeout cannot land on the destructive half.",
      inputSchema: z
        .object({
          endpoint_id: z.number().int().describe("Endpoint ID"),
          container_id: z.string().describe("Container ID or name"),
