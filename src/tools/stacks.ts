...
export function registerStackTools(
  ...
  server.registerTool(
    "portainer_convert_stack_to_file",
    {
      ...
      description:
        "Convert an existing git-managed standalone Compose stack into a file-based stack, permanently removing Git repository authentication from that stack's deployment path. Reads the checked-out compose and real environment values server-side, requires two-factor confirmation, then sends them through Portainer's file-update endpoint on the SAME stack record. This preserves the stack ID, name, endpoint, access controls, compose, and every environment value while Portainer clears GitConfig in place; no delete/recreate window exists and secret values never enter the tool response. The update redeploys the stack without forcing an image pull. Portainer Git polling and Git auto-update settings are intentionally removed; future compose changes use portainer_update_stack_file and image-only refreshes use portainer_redeploy_stack. Supports standalone Compose stacks (Type 2) only. SELF-CONVERSION WARNING: do not run this against the portainer-mcp stack itself because the call will be interrupted when its own container restarts.",
      inputSchema: z
        .object({
          source_stack_id: z
            .number()
